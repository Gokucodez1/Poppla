...
from bit import PrivateKey  # Optional — will simulate if not found
    ...
    while not bot.is_closed():
        for ch_id, data in list(channel_data.items()):
            ...
            if data.get("amount") and not data.get("txid"):
                ...
                try:
                    url = f"https://sochain.com/api/v2/get_address_balance/LTC/mainnet/ {ltc_address}"
                    response = requests.get(url)
                    data = response.json()

                    if float(data["data"]["received_value"]) > 0:
                        channel_data[ch_id]["txid"] = data["data"]["transactions"][0]
...
async def process_auto_release(channel, ch_id):
    ...
    try:
        ...
        txid = key.send([(ltc_address, amount_btc, 'btc')])
...
async def send_role_selection(channel):
    ...
    view.add_item(Button(label="Sending", custom_id="sending"))
    view.add_item(Button(label="Receiving", custom_id="receiving"))
    view.add_item(Button(label="Reset", style=discord.ButtonStyle.danger, custom_id="reset"))
...
async def send_confirmation(channel):
    ...
    view = View(timeout=None)
    view.add_item(Button(label="Correct", style=discord.ButtonStyle.green, custom_id="correct"))
    view.add_item(Button(label="Incorrect", style=discord.ButtonStyle.red, custom_id="incorrect"))
...
async def send_payment_invoice(channel):
    ...
    view = View(timeout=None)
    view.add_item(Button(label="Paste", custom_id="paste"))
    view.add_item(Button(label="Scan QR", custom_id="qr"))
